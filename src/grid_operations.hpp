...
class grid_ops {
 public:
  static bool has_collisions(grid_span const& where) {
    ...
    return std::any_of(digit_count.begin(), digit_count.end(),
                       [](auto c) { return c > 1; });
  }

  static bool is_grid_invalid(grid const& g) {
    auto invalid = std::any_of(g.cbegin(), g.cend(),
                               [](auto& c) { return c.is_invalid(); });

    // Check for doubles in inner boxes
    if (!invalid) {
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(0, 0, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(3, 0, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(6, 0, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(0, 3, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(3, 3, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(6, 3, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(0, 6, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(3, 6, g.data()));
      invalid =
          invalid || has_collisions(grid_ops::get_inner_box(6, 6, g.data()));
    }
  ...
  static bool only_two_possible(cell const& c) { return c.count() == 2; }
  ...
  static bool reset_if_last_possible(grid_span where, cell& c) {
    ...
    for (int i = 1; i <= 9; ++i) {
      if (c.possibility_collides(i) &&
          std::none_of(where.begin(), where.end(),
                       std::bind(is_colliding, _1, i))) {
        c.reset_to(i);
        return true;
      }
    }
  ...
  static grid_span get_column(int c, grid::const_pointer ptr,
                              grid::size_type except = -1) {
    auto s = calculate_index(0, c);
    indices indices = {
        index_or_skip(s, except),      index_or_skip(s + 9, except),
        index_or_skip(s + 18, except), index_or_skip(s + 27, except),
        index_or_skip(s + 36, except), index_or_skip(s + 45, except),
        index_or_skip(s + 54, except), index_or_skip(s + 63, except),
        index_or_skip(s + 72, except)};
    return grid_span{grid_iterator{ptr, std::move(indices)}, end_iter()};
  }

  /** Get a range that describes a given row, given `r` between 0 and 8 */
  static grid_span get_row(int r, grid::const_pointer ptr,
                           grid::size_type except = -1) {
    auto s = calculate_index(r, 0);
    indices indices = {
        index_or_skip(s, except),     index_or_skip(s + 1, except),
        index_or_skip(s + 2, except), index_or_skip(s + 3, except),
        index_or_skip(s + 4, except), index_or_skip(s + 5, except),
        index_or_skip(s + 6, except), index_or_skip(s + 7, except),
   ...
   *  are between 0 and 8 */
  static grid_span get_inner_box(int r, int c, grid::const_pointer ptr,
                                 grid::size_type except = -1) {
    int sRow = 3 * (r / 3);
    int sCol = 3 * (c / 3);
    ...
    indices indices = {
        index_or_skip(s, except),      index_or_skip(s + 1, except),
        index_or_skip(s + 2, except),  index_or_skip(s + 9, except),
        index_or_skip(s + 10, except), index_or_skip(s + 11, except),
        index_or_skip(s + 18, except), index_or_skip(s + 19, except),
        index_or_skip(s + 20, except)};
    return grid_span{grid_iterator{ptr, std::move(indices)}, end_iter()};
  }

  /** Return i unless i == except, in which case return -1 */
  static constexpr grid::size_type index_or_skip(grid::size_type i,
                                                 grid::size_type except) {
    return ((i == except) ? -1 : i);
  }
